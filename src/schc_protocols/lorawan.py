...
class LoRaWAN(SCHCProtocol):
    ...
    def __set_parameters__(self):
        if self.RULE_ID == 0:
            ...
            self.MAX_ACK_REQUEST = 1e6 
            self.INACTIVITY_TIMER = 300  # in seconds
            self.RETRANSMISSION_TIMER = 30  # in seconds
        elif self.RULE_ID == LoRaWAN.ACK_ALWAYS:  # Downlink data transfer
            self.T = 0  # in bits
            self.M = 1  # in bits # WINDOW
            self.N = 1  # in bits # FCN
            self.U = 32  # in bits
            self.WINDOW_SIZE = 1  # 2^(n=6) = 64 - {All-1 fragment} # 1 tile
            self.TILE_SIZE = 10 * 8  # 10 bytes = 80 bits
            self.MAX_ACK_REQUEST = 8 
            self.INACTIVITY_TIMER = 60 * 5  # in seconds
            self.RETRANSMISSION_TIMER = 60 * 3  # in seconds
    ...
    def payload_condition_all1(self, payload):
        ...
        if payload == "":
            ...
            if self.RULE_ID == LoRaWAN.ACK_ON_ERROR:
                ...
                return payload # REVISAR - NÃO ALTERADO
    ...
    def penultimate_tile(self):
        ...
        # else:
        #     return 0
        return self.TILE_SIZE # REVISAR - ALTERADO
