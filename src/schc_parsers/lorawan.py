...
from schc_messages import SCHCMessage, SCHCAck, SCHCAckReq, SCHCSenderAbort, SCHCReceiverAbort, All1SCHCFragment, RegularSCHCFragment
from schc_protocols import LoRaWAN

def parse(message, side):
    """
    ...
    if rule_id == LoRaWAN.ACK_ON_ERROR:
        return __parse_ack_on_error__(message, side)
    elif rule_id == LoRaWAN.ACK_ALWAYS:
        return __parse_ack_always__(message, side)
...
def __parse_ack_on_error__(message, side):
    """
...
def __parse_ack_always__(message, side):
    """
    ...
    assert length % protocol.L2_WORD == 0, "Bits received does not match L2 word"
    pointer = protocol.RULE_SIZE + protocol.T + protocol.M
    bits_to_check = bits_received[pointer:pointer + 1]
    ...
    if length == protocol.L2_WORD * 2:
        if side == "receiver":
            if bits_to_check == "0" * protocol.N:
                ...
                return SCHCSenderAbort.from_bytes(message, protocol=protocol.id)
        elif side == "sender":
            return SCHCAck.from_bytes(message, protocol=protocol.id)
    elif length == protocol.L2_WORD * 3:
        if bits_to_check == "0":
            return RegularSCHCFragment.from_bytes(message, protocol=protocol.id)
        elif bits_to_check == "1":
            return SCHCReceiverAbort.from_bytes(message, protocol=protocol.id)
    elif length > protocol.L2_WORD * 3:
        if bits_to_check == "0":
            return RegularSCHCFragment.from_bytes(message, protocol=protocol.id)
